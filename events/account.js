...
module.exports = function (server, io, db) {
    ...
    this.onEnterWorld = function (data, socket) {
        ...
        {
            db.query("SELECT id,name,icon,type,castTime,resource,sParticles,dParticles FROM characterspells as cs INNER JOIN spell as s ON cs.spell_id = s.id WHERE cs.character_id = ?", tmpCharacter.id).on("result", function (spell) {
                var data = {
                    id: spell.id,
                    name: spell.name,
                    icon: spell.icon,
